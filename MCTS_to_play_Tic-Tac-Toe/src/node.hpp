...
// define three classes: class StateMachine, class Searcher,class Node
namespace mcts {
...
class StateMachine {
    public:
      constexpr StateMachine(
        ...
        ActionToStringFunc<ACTION> action_to_string) : //initilize
...
// class Node
template< typename STATE, typename ACTION, int ROLE_COUNT, const StateMachine<STATE, ACTION, ROLE_COUNT>& SM >
class Node {
...
// class Searcher
template< class STATE, class ACTION, int ROLE_COUNT, const StateMachine<STATE, ACTION, ROLE_COUNT>& SM >
class Searcher {
  using NodeType = Node<STATE, ACTION, ROLE_COUNT, SM>;
    public:
